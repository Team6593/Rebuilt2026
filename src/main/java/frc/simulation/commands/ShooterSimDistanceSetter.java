// Copyright (c) FIRST and other WPILib contributors.
// Open Source Software; you can modify and/or share it under the terms of
// the WPILib BSD license file in the root directory of this project.

package frc.simulation.commands;

import edu.wpi.first.wpilibj2.command.InstantCommand;
import frc.simulation.shooter.ShooterSimulation;

// NOTE:  Consider using this command inline, rather than writing a subclass.  For more
// information, see:
// https://docs.wpilib.org/en/stable/docs/software/commandbased/convenience-features.html
public class ShooterSimDistanceSetter extends InstantCommand {

  private ShooterSimulation shooterSimulation;
  private double distance;

  public ShooterSimDistanceSetter(ShooterSimulation shooterSimulation, double distance) {
    this.shooterSimulation = shooterSimulation;
    this.distance = distance;

    addRequirements(shooterSimulation);
    // Use addRequirements() here to declare subsystem dependencies.
  }

  // Called when the command is initially scheduled.
  @Override
  public void initialize() {
    shooterSimulation.setDistance(distance);
  }
}
